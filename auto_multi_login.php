...
// массив "$urls" содержит WEB-адреса хостов которые будем обрабатывать;
// "$global_task" - задача которую будет выполнять WEB-приложение;
// 5 параметр: если - TRUE - включаем отладку;
// 6 параметр: если - TRUE - включаем запись отладки в лог-файл.
$rcMcURL = new RoundcubeMultiCURL($user, $password, $urls, $global_task, TRUE, TRUE);
...
// Если включен режим отладки (debugEnabled = TRUE).
if($rcMcURL->debugEnabled == TRUE){
echo 'rcMcURL->content_h[post]: ', "\n";
var_dump($rcMcURL->content_h['post']);
}

// Закрываем набор cURL-дескрипторов.
$rcMcURL->curl_multi_close();

// Завершаем работу скрипта. exit; // Выполнение парарельных запросов в cURL. //$rcMcURL->
    curl_multi_exec();
    ...
    //// В цикле выполняем запросы пока объект "$rcMcURL" содержит массив "$curls".
    //do{
  // // Выполняем установку параметров cURL для POST-запроса на выполнение // // команды WEB-серверу в Roundcube. // $rcMcURL->curl_multi_set_request_post();
        //
        // // Выполнение парарельных запросов в cURL.
        // // POST-запрос на выполнение команды WEB-серверу:
        // $rcMcURL->curl_multi_exec();
        ...
        // * как ^, $ или (?<=x). Вы можете найти соответствующие примеры в описании // * функции preg_match(). // *
            Возвращаемые значения: // * Возвращает количество найденных вхождений шаблона (которое может быть // * и
            нулём) либо false, если во время выполнения возникли какие-либо ошибки. // */ // // Получаем
            строку "Set-Cookie" из результатов выдачи сервера. // preg_match_all('/^Set-Cookie:.*/im', $response,
            $resultC); // /** // * Условный оператор ?, возвращает y, в случае если x принимает значение true, // * и z
            в случае, если x принимает значение false. Пример: x ? y : z. // */ // // Если массив "$resultX" сформирован
            - то переменной "$processing" // // присвоим TRUE иначе FALSE. // $processing=$resultX ? TRUE : FALSE; // //
            Если массив "$resultC" с элементом "['0']" сформирован - то переменной // // "$set_cookie" присвоим TRUE
            иначе FALSE. // $set_cookie=$resultC['0'] ? TRUE : FALSE; // // // Если переменные "$processing"
            и "$set_cookie" равны FALSE: // if($processing==FALSE & $set_cookie==FALSE){ // // В массив "$del_urls"
            добавляем адрес домена который нужно // // исключить из обработки. // $del_urls[$url]=$url; // } // // //
            Если переменная "$set_cookie" равна TRUE: // if($set_cookie==TRUE){ // // В терминал выводим сообщение о
            выполнении процедуры // // перерегистрации в Roundcube: // // дата 2001.03.10 17:16:18 (формат MySQL
            DATETIME); // $new_reg=date("Y.m.d") . " " ; // // время (17:16:18); // $new_reg .=date("H:i:s") . "\t" ; //
            // сообщение; // $new_reg .="RC multi cURL new registration on: $url \n" ; // // выводим сообщение; //
            ...
            // $rcMcURL->write_log_file($msg_stop);
            //}
// Сообщение об окончании работы программы. // В условии проверяем если включен режим отладки
                (debugEnabled=TRUE) и // указание записи в лог-файл (writeLogEnabled==TRUE): // вызываем функцию записи
                в лог-файл. if($rcMcURL->writeLogEnabled == TRUE){
                // Дата 2001.03.10 17:16:18 (формат MySQL DATETIME).
                $msg_stop = date("Y.m.d") . " ";
                // Время (17:16:18).
                $msg_stop .= date("H:i:s") . "\t";
...
// * preg_match_all — Выполняет глобальный поиск шаблона в строке.
// * Описание:
// * preg_match_all(string $pattern, string $subject, array &$matches=null, int
$flags=PREG_PATTERN_ORDER, int $offset=0):int|false|null
// *
// * Ищет в строке subject все совпадения с шаблоном pattern и помещает
// * результат в массив matches в порядке, * определяемом комбинацией
// * флагов flags.
...
// * как ^, $ или (?<=x). Вы можете найти соответствующие примеры в описании // * функции preg_match(). // *
    ...
    запишем это сообщение в лог-файл; // $rcMcURL->
    write_log_file($new_reg);
    // // выполняем установку параметров cURL для авторизации в Roundcube;
    // $rcMcURL->curl_multi_get_auth();
    // // выполнение парарельных запросов в cURL;
    // $rcMcURL->curl_multi_exec();
    ...
    //
    // // Удаляем элемент массива по ключу.
    // unset($rc_curls[$url], $rc_tokens[$url], $rc_content_h['auth'][$url],
    $rc_content_h['post'][$url]);
    //
    // /**
    // * array_search — Осуществляет поиск данного значения в массиве и
    // * возвращает ключ первого найденного элемента в случае успешного выполнения.
