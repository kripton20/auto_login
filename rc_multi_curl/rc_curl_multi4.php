...
foreach ($urls as $key=>$url) {
    ...
    //curl_setopt($ch[$key], CURLOPT_COOKIE, $cookies_my[$key]);
    //$k = $cookies_my['0'];
...
do {
    ...
    */
    ...
    //    // один из потоков завершил работу
    //    if ($running == 0) {
    ...
    //                */
    //                $CURL_OPERATIONS_auth[$key] = curl_getinfo($info['handle']);
    //                $CURLINFO_COOKIELIST_auth[$key] = curl_getinfo($info['handle'], CURLINFO_COOKIELIST);
...
foreach ($ch as $key=>$value) {
    ...
    */
    ...
    $CURL_CONTENT_AUTH[$key] = curl_multi_getcontent($ch[$key]);
    /**
    * curl_getinfo — Возвращает информацию об определённой операции.
    *
    * curl_getinfo(CurlHandle $handle, ?int $option=null):mixed
    ...
    * https://www.php.net/manual/ru/function.curl-getinfo.php
    */
    $CURL_INFO_AUTH[$key] = curl_getinfo($value);
    /**
    ...
    */
    // Получаем токен.
    preg_match('/^Location:\s\.\/\?_task=mail&_token=([A-Za-z0-9]+)[^\\n]/im', $CURL_CONTENT_AUTH[$key], $token);
    // Запишем в массив "$tokens" полученные токены.
    $tokens[$key] = $token['1'];

    /**
    * Иногда вы не можете использовать CURLOPT_COOKIEJAR и CURLOPT_COOKIEFILE из-за php-настроек сервера (говорят, что вы можете получить любые файлы с сервера, используя эти параметры). Вот решение
    * 1) Не используйте CURLOPT_FOLLOWLOCATION
    * 2) Используйте curl_setopt($ch, CURLOPT_HEADER, 1)
    ...
    */
    preg_match_all('|Set-Cookie: (.*);|U', $CURL_CONTENT_AUTH[$key], $results);
    $cookies[$key] = implode(';', $results[1]);
    //preg_match('/^Set - Cookie:\s(roundcube_sessid = [A - Za - z0 - 9] + )[ ^ \s] / im', $CURL_CONTENT_AUTH[$key], $results2);
    //$cookies_my[$key] = $results2[1];
...
do {
    ...
    */
    ...
    //            $info = curl_multi_info_read($mh);
    //            while ($info) {
    //                $CURL_OPERATIONS_GET1[$key] = curl_getinfo($info['handle']);
    //                $CURLINFO_COOKIELIST_GET1[$key] = curl_getinfo($info['handle'], CURLINFO_COOKIELIST);
    //                //echo var_dump($CURL_OPERATIONS['request_header']);
    //                //echo var_dump($CURL_OPERATIONS['url']);
    //                break;
    //            }
...
foreach ($ch as $key=>$value) {
    ...
    */
    // Получаем ответ от сервера.
    $CURL_CONTENT_GET1[$key] = curl_multi_getcontent($ch[$key]);
    /**
    * curl_getinfo — Возвращает информацию об определённой операции.
    *
    * curl_getinfo(CurlHandle $handle, ?int $option=null):mixed
    ...
    * https://www.php.net/manual/ru/function.curl-getinfo.php
    */
    $CURL_INFO_GET1[$key] = curl_getinfo($value);
...
do {
    ...
    */
    ...
    //            $info = curl_multi_info_read($mh);
    //            while ($info) {
    //                $CURL_OPERATIONS_GET2[$key] = curl_getinfo($info['handle']);
    //                $CURLINFO_COOKIELIST_GET2[$key] = curl_getinfo($info['handle'], CURLINFO_COOKIELIST);
    //                //echo var_dump($CURL_OPERATIONS['request_header']);
    //                //echo var_dump($CURL_OPERATIONS['url']);
    //                break;
    //            }
...
foreach ($ch as $key=>$value) {
    ...
    */
    // Получаем ответ от сервера.
    $CURL_CONTENT_GET2[$key] = curl_multi_getcontent($ch[$key]);
    /**
    * curl_getinfo — Возвращает информацию об определённой операции.
    *
    * curl_getinfo(CurlHandle $handle, ?int $option=null):mixed
    ...
    * https://www.php.net/manual/ru/function.curl-getinfo.php
    */
    $CURL_INFO_GET2[$key] = curl_getinfo($value);
...
do {
    ...
    */
    ...
    //            $info = curl_multi_info_read($mh);
    //            while ($info) {
    //                $CURL_OPERATIONS_POST[$key] = curl_getinfo($info['handle']);
    //                $CURLINFO_COOKIELIST_POST[$key] = curl_getinfo($info['handle'], CURLINFO_COOKIELIST);
    //                //echo var_dump($CURL_OPERATIONS['request_header']);
    //                //echo var_dump($CURL_OPERATIONS['url']);
    //                break;
    //            }
...
foreach ($ch as $key=>$value) {
    ...
    */
    // Получаем ответ от сервера.
    $CURL_CONTENT_POST[$key] = curl_multi_getcontent($ch[$key]);
    /**
    * curl_getinfo — Возвращает информацию об определённой операции.
    *
    * curl_getinfo(CurlHandle $handle, ?int $option=null):mixed
    ...
    * https://www.php.net/manual/ru/function.curl-getinfo.php
    */
    $CURL_INFO_POST[$key] = curl_getinfo($value);
...
do {
    ...
    */
    ...
    //            $info = curl_multi_info_read($mh);
    //            while ($info) {
    //                $CURL_OPERATIONS_GET[$key] = curl_getinfo($info['handle']);
    //                $CURLINFO_COOKIELIST_GET[$key] = curl_getinfo($info['handle'], CURLINFO_COOKIELIST);
    //                //echo var_dump($CURL_OPERATIONS['request_header']);
    //                //echo var_dump($CURL_OPERATIONS['url']);
    //                break;
    //            }
...
foreach ($ch as $key=>$value) {
    ...
    */
    // Получаем ответ от сервера.
    $CURL_CONTENT_GET_AUT[$key] = curl_multi_getcontent($ch[$key]);
    /**
    * curl_getinfo — Возвращает информацию об определённой операции.
    *
    * curl_getinfo(CurlHandle $handle, ?int $option=null):mixed
    ...
    * https://www.php.net/manual/ru/function.curl-getinfo.php
    */
    $CURL_INFO_GET_AUT[$key] = curl_getinfo($value);
