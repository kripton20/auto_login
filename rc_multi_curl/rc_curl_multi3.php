...
foreach ($ch as $key=>$value) {
	...
	*/
	preg_match_all('|Set-Cookie: (.*);|U', $headers1_auth[$key], $results);
	$cookies[$key] = implode(';', $results[1]);
	//preg_match('/^Set-Cookie:\s(roundcube_sessid=[A-Za-z0-9]+)[^\s]/im', $headers1_auth[$key], $results2);
...
// В цикле перебираем элементы массива "$ch" получаем содержимое ответа сервера.
foreach ($ch as $key=>$value) {
    $headers_GET1[$key] = curl_multi_getcontent($ch[$key]);
    curl_multi_remove_handle($mh, $ch[$key]);
}
// 2 GET
foreach ($urls as $key=>$url) {
...
// В цикле перебираем элементы массива "$ch" получаем содержимое ответа сервера.
foreach ($ch as $key=>$value) {
    $headers_GET2[$key] = curl_multi_getcontent($ch[$key]);
    curl_multi_remove_handle($mh, $ch[$key]);
}
#################
...
// В цикле перебираем элементы массива "$ch" получаем содержимое ответа сервера.
foreach ($ch as $key=>$value) {
    $headers3_GET[$key] = curl_multi_getcontent($ch[$key]);
    curl_multi_remove_handle($mh, $ch[$key]);
}
#################
