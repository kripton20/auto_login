...
class RoundcubeMultiCURL
{
    ...
    *
    * @param array $content_get_aut
    */
    public $content_get_aut;
    ...
    */
    ...
    public function __construct($urls, $user, $password, $debugEnabled = FALSE, $writeLogEnabled = FALSE)
    {
        ...
        // Получаем контент данные GET-запроса на выход получаемые от сервера.
        $this->content_get_aut = array();
        // Получаем контент данные GET-запроса на выход отправляемые серверу.
        $this->content_get_aut_info = array();
        // Получаем куки с сервера
        $this->cookies = array();
        // Запишем в массив "$tokens" полученные токены.
        $this->tokens = array();
    ...
    */
    public function auth()
    {
        ...
        foreach ($this->urls as $key=>$url) {
            ...
            curl_setopt($this->curls[$key], CURLOPT_AUTOREFERER, TRUE);
            ...
            curl_setopt($this->curls[$key], CURLOPT_POST, TRUE);
            ...
            curl_setopt($this->curls[$key], CURLOPT_POSTFIELDS, '_task=login&_action=login&_timezone=Europe/Moscow&_url=&_user=' . $this->user . '&_pass=' . $this->password);
            // CURLOPT_URL    Загружаемый URL. Данный параметр может быть также установлен при инициализации сеанса с помощью curl_init().
            curl_setopt($this->curls[$key], CURLOPT_URL, $url . '?_task=login');
            ...
            curl_multi_add_handle($this->multiCurl, $this->curls[$key]);
        }
    }
    ...
    */
    ...
    public function curl_multi_getcontent_auth()
    {
        ...
        foreach ($this->curls as $key=>$value) {
            ...
            */
            // Получаем данные авторизации получаемые от сервера.
            $this->content_auth[$key] = curl_multi_getcontent($this->curls[$key]);
            ...
            */
            ...
            $this->content_auth_info[$key] = curl_getinfo($value);
            ...
            */
            preg_match_all('/Set-Cookie: (.*);/U', $this->content_auth[$key], $results);
        ...
        foreach ($this->urls as $key=>$url) {
            ...
            */
            ...
            (
                ...
                'X-Roundcube-Request: ' . $this->tokens[$key],
                'X-Compress: null',
            );
            ...
            curl_setopt($this->curls[$key], CURLOPT_NOBODY, TRUE);
            curl_setopt($this->curls[$key], CURLINFO_HEADER_OUT, TRUE);
            // CURLOPT_HTTPHEADER    Массив устанавливаемых HTTP - заголовков,
            // в формате array('Content - type: text / plain', 'Content - length: 100')
            curl_setopt($this->curls[$key], CURLOPT_HTTPHEADER, $post_headers);
            curl_setopt($this->curls[$key], CURLOPT_RETURNTRANSFER, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_HEADER, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_URL, $url . '?_task=mail&_action=plugin.msg_request');
            curl_setopt($this->curls[$key], CURLOPT_POSTFIELDS, '_remote=1&_unlock=0');
            ...
            curl_setopt($this->curls[$key], CURLOPT_COOKIE, $this->cookies[$key]);
            curl_setopt($this->curls[$key], CURLOPT_USERAGENT, 'curl/7.81.0');
            curl_multi_add_handle($this->multiCurl, $this->curls[$key]);
        }
    ...
    // Получаем контент POST - запроса.
    public function curl_multi_getcontent_rm_post()
    {
        // В цикле перебираем элементы массива "$ch" получаем содержимое ответа сервера.
        foreach ($this->curls as $key=>$value) {
            // Получаем контент POST - запроса.
            $this->content_post[$key] = curl_multi_getcontent($this->curls[$key]);
            // Получаем данные POST - запроса отправляемые серверу.
            $this->content_post_info[$key] = curl_getinfo($value);
            // Закрываем все дескрипторы.
            curl_multi_remove_handle($this->multiCurl, $this->curls[$key]);
        }
    }
    ...
    public function get_aut()
    {
        ...
        foreach ($this->urls as $key=>$url) {
            curl_setopt($this->curls[$key], CURLOPT_AUTOREFERER, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_HTTPGET, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_URL, $url . '?_task=logout&_token=' . $this->tokens[$key]);
            curl_setopt($this->curls[$key], CURLOPT_VERBOSE, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_NOBODY, TRUE);
            curl_setopt($this->curls[$key], CURLINFO_HEADER_OUT, TRUE);
            curl_setopt($this->curls[$key], CURLOPT_HEADER, TRUE);
    ...
    // GET - данные на выход:
    public function curl_multi_getcontent_aut()
    {
        // В цикле перебираем элементы массива "$ch" получаем содержимое ответа сервера.
        foreach ($this->curls as $key=>$value) {
            // Получаем ответ от сервера.
            $this->content_get_aut[$key] = curl_multi_getcontent($this->curls[$key]);
            $this->content_get_aut_info[$key] = curl_getinfo($value);
            // Закрываем все дескрипторы.
            curl_multi_remove_handle($this->multiCurl, $this->curls[$key]);
        }
    }
